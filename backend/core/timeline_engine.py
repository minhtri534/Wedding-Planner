...
from .models import UserPreferences, TimelineTask, WeddingTimeline
...
class TimelineEngine:
    def generate_timeline(self, prefs: UserPreferences) -> WeddingTimeline:
        ...
        for due_month, cat, name, crit in all_tasks:
            if due_month > months:
                 # Task was due earlier ideally. Do it now.
                 prefix = "[URGENT]" if months < 6 else "[OVERDUE]"
                 tasks.append(TimelineTask(month_due=months, task_name=f"{prefix} {name}", category=cat, is_critical=True))
            else:
