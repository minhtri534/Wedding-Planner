...
from app.schemas.project import ConceptVibe, Season
...
class ChatEngine:
    def process_query(self, query: str, context: Dict) -> str:
        ...
        if "budget" in query or "cost" in query or "afford" in query:
            ...
            return f"Your total budget is ${budget:,.0f} for {guests} guests. That's about ${budget/guests:,.0f} per guest, which is a {self._get_budget_tier(budget, guests)} budget."
